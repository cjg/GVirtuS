...
size_t
TcpCommunicator::Read(char *buffer, size_t size) {
...
size_t
TcpCommunicator::Write(const char *buffer, size_t size) {
...
void
TcpCommunicator::Sync() {
  mpOutput->flush();
}

void
TcpCommunicator::InitializeStream() {
#ifdef _WIN32
...
create_communicator(std::shared_ptr<Endpoint> end) {
  std::string arg = "tcp://" + std::dynamic_pointer_cast<Endpoint_Tcp>(end)->address() + ":"
      + std::to_string(std::dynamic_pointer_cast<Endpoint_Tcp>(end)->port());
