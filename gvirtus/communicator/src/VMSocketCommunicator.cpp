...
namespace gvirtus::comm {
  ...
  size_t
  VMSocketCommunicator::Read(char *buffer, size_t size) {
    size_t offset = 0;
    int readed;
    while (offset < size) {
  ...
  size_t
  VMSocketCommunicator::Write(const char *buffer, size_t size) {
    size_t offset = 0;
    int written;
    while (offset < size) {
