...
const Communicator * const ZmqCommunicator::Accept() const {
    ...
    return new ZmqCommunicator(client_socket_fd,
            inet_ntoa(client_socket_addr.sin_addr));
}

void ZmqCommunicator::Connect() {
...
void ZmqCommunicator::Close() {
    throw "ZmqCommunicator::Close()\n";
}

size_t ZmqCommunicator::Read(char* buffer, size_t size) {
...
size_t ZmqCommunicator::Write(const char* buffer, size_t size) {
...
void ZmqCommunicator::Sync() {
    mpOutput->flush();
}

void ZmqCommunicator::InitializeStream() {
#ifdef _WIN32
