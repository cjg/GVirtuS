...
int main() {
	...
	char chBuffer[10240];
    	cl_platform_id clPlatformIDs[100];
	...
	printf("size of platformID %d,  size of num_platforms : %d", sizeof(platform), sizeof(platforms));
	...
	error = clGetPlatformInfo(clPlatformIDs[0], CL_PLATFORM_NAME, 10240, &chBuffer, NULL);
	cl_context_properties properties[]={
		CL_CONTEXT_PLATFORM, (cl_context_properties)platform,
		0};
	// Note that nVidia's OpenCL requires the platform property
	cl_context context=clCreateContext(properties, 1, &device, NULL, NULL, &error);
	printf("cl_context %lu\n", (unsigned long)context);

	size_t nDeviceBytes;
	error = clGetContextInfo(context, CL_CONTEXT_DEVICES, 0, NULL, &nDeviceBytes);
