...
namespace gvirtus {
  ...
  VmciCommunicator::Serve() {
    ...
    if (bind(mSocketFd, (struct sockaddr *)&socket_addr, sizeof(struct sockaddr_vm)) != 0)
  ...
  VmciCommunicator::Connect() {
    ...
    if (connect(mSocketFd, (struct sockaddr *)&remote, sizeof(struct sockaddr_vm)) != 0)
      throw "VmciCommunicator: Can't connect to socket.";
    InitializeStream();
  }

  void
  VmciCommunicator::Close() {}

  std::istream &
  VmciCommunicator::GetInputStream() const {
    return *(this->mpInput);
  }

  std::ostream &
  VmciCommunicator::GetOutputStream() const {
    return *(this->mpOutput);
  }

  void
  VmciCommunicator::InitializeStream() {
    mpInputBuf = new __gnu_cxx::stdio_filebuf<char>(mSocketFd, std::ios_base::in);
    mpOutputBuf = new __gnu_cxx::stdio_filebuf<char>(mSocketFd, std::ios_base::out);
